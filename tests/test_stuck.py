...
class TestStuckDetector:
    ...
    def test_stationary_becomes_stuck(self) -> None:
        ...
        assert sd.check(_pt(100.0, 100.0)) is False
        t[0] = 100.1
        assert sd.check(_pt(100.0, 100.0)) is False
        t[0] = 100.2
        result = sd.check(_pt(100.0, 100.0))
    ...
    def test_moving_clears_stuck(self) -> None:
        ...
        for i in range(4):
            t[0] = 100.0 + i * 0.1
            sd.check(_pt(100.0, 100.0))
        assert sd.is_stuck is True
        # Move far enough to clear
        t[0] = 102.0
        result = sd.check(_pt(200.0, 200.0))
    ...
    def test_zero_speed_detection(self) -> None:
        ...
        t = [100.0]
        sd = StuckDetector(clock=lambda: t[0])
        sd.check(_pt(100.0, 100.0), speed=0.0)
        t[0] = 100.1
        result = sd.check(_pt(100.0, 100.0), speed=0.0)
        assert result is True
    ...
    def test_negative_speed_skips_speed_check(self) -> None:
        ...
        result = sd.check(_pt(100.0, 100.0), speed=-1.0)
        assert result is False
    ...
    def test_is_stuck_property(self) -> None:
        ...
        sd.check(_pt(50.0, 50.0), speed=0.0)
        t[0] = 100.1
        sd.check(_pt(50.0, 50.0), speed=0.0)
        assert sd.is_stuck is True
    ...
    def test_speed_nonzero_resets_counter(self) -> None:
        ...
        sd.check(_pt(120.0, 120.0), speed=5.0)
        t[0] = 100.2
        # Next zero-speed tick is only the first again, so not stuck via speed
        sd.check(_pt(140.0, 140.0), speed=0.0)
